...
async function verifierSession() {
  ...
  if (accessToken && type === 'recovery') {
    await db.auth.setSession({ access_token: accessToken, refresh_token: hashParams.get('refresh_token') })
  ...
  if (data.session) {
    ...
    if (stravaCode) { window.history.replaceState({}, document.title, '/'); recupererTokenStrava(stravaCode) }
  } else {
...
async function sInscrire() {
  ...
  if (error) { document.getElementById('register-error').style.display = 'block'; return }
...
async function envoyerResetPassword() {
  ...
  if (!email) return
  const { error } = await db.auth.resetPasswordForEmail(email, { redirectTo: 'https://kmc-team.vercel.app' })
  if (error) { document.getElementById('forgot-error').style.display = 'block'; document.getElementById('forgot-success').style.display = 'none'; return }
...
async function resetPassword() {
  ...
  if (newPassword !== confirmPassword) { document.getElementById('reset-error').style.display = 'block'; return }
  const { error } = await db.auth.updateUser({ password: newPassword })
  if (error) { document.getElementById('reset-error').textContent = 'Erreur — réessayez.'; document.getElementById('reset-error').style.display = 'block'; return }
  document.getElementById('reset-success').style.display = 'block'
  document.getElementById('reset-error').style.display = 'none'
  setTimeout(() => { document.getElementById('reset-page').style.display = 'none'; document.getElementById('login-page').style.display = 'flex' }, 2000)
}

async function afficherProfil() {
  const { data } = await db.auth.getSession()
  if (data.session) document.getElementById('user-email').textContent = data.session.user.email
}
...
async function changerEmail() {
  ...
  if (error) { document.getElementById('email-error').style.display = 'block'; return }
...
async function changerMotDePasse() {
  ...
  if (!password) return
  if (password !== confirm) { document.getElementById('password-error').textContent = 'Les mots de passe ne correspondent pas.'; document.getElementById('password-error').style.display = 'block'; return }
  const { error } = await db.auth.updateUser({ password })
  if (error) { document.getElementById('password-error').textContent = 'Erreur — réessayez.'; document.getElementById('password-error').style.display = 'block'; return }
...
async function chargerMembres() {
  const { data, error } = await db.from('membres').select('*').order('km_mois', { ascending: false })
  if (error) { console.error('Erreur:', error); return }
  window.tousLesMembres = data
  window.tousLesMembresOriginal = data
...
function afficherPageMembres(page) {
  ...
  const pageDonnees = data.slice(debut, debut + parPage)
...
function filtrerMembres(texte) {
  if (!window.tousLesMembresOriginal) return
  if (texte === '') { window.tousLesMembres = window.tousLesMembresOriginal; afficherPageMembres(1); return }
  window.tousLesMembres = window.tousLesMembresOriginal.filter(m => m.nom.toLowerCase().includes(texte.toLowerCase()))
  afficherPageMembres(1)
}
...
async function modifierPoints(id, pointsActuels) {
  ...
  const { error } = await db.from('membres').update({ points: parseInt(nouveau) }).eq('id', id)
  if (error) { console.error(error); return }
  chargerMembres()
  chargerClassement()
  chargerClassementPoints()
...
function ouvrirModalModif(id, nom, role, specialite, dob, categorie, km, photoUrl, licence) {
  ...
  if (photoUrl) {
    ...
    preview.innerHTML = nom ? nom.split(' ').map(w => w[0]).join('').slice(0,2).toUpperCase() : '?'
  }
  document.getElementById('modal-membre').style.display = 'flex'
}

function fermerModal() { document.getElementById('modal-membre').style.display = 'none'; modalPhotoFile = null }

function previewPhoto(input) {
  if (!input.files[0]) return
  modalPhotoFile = input.files[0]
  const reader = new FileReader()
  reader.onload = e => { document.getElementById('modal-avatar-preview').innerHTML = `<img src="${e.target.result}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">` }
...
async function sauvegarderModal() {
  ...
  if (!nom) { document.getElementById('modal-error').style.display = 'block'; return }
  let photoUrl = null
  if (modalPhotoFile) {
    const nomFichier = id ? `${id}.jpg` : `${Date.now()}.jpg`
    const { error: uploadError } = await db.storage.from('photos').upload(nomFichier, modalPhotoFile, { upsert: true, contentType: modalPhotoFile.type })
    if (!uploadError) {
      const { data } = db.storage.from('photos').getPublicUrl(nomFichier)
      photoUrl = `${data.publicUrl}?t=${Date.now()}`
    }
  }
  const payload = { nom, role: role || 'Equipier', specialite: specialite || null, date_naissance: dob || null, categorie: categorie || null, num_licence: licence || null }
  if (photoUrl) payload.photo_url = photoUrl
  if (id) { const { error } = await db.from('membres').update(payload).eq('id', id); if (error) { console.error(error); return } }
  else { const { error } = await db.from('membres').insert([payload]); if (error) { console.error(error); return } }
  fermerModal()
  chargerMembres()
  chargerClassement()
  chargerClassementPoints()
...
async function chargerEvenements() {
  const { data, error } = await db.from('evenements').select('*').order('date', { ascending: true })
  if (error) { console.error('Erreur:', error); return }
  ...
  data.forEach(evt => {
    ...
    const dateAffichee = `${date.getDate().toString().padStart(2,'0')}-${(date.getMonth()+1).toString().padStart(2,'0')}-${date.getFullYear()}`
    ...
    <div class="event-info"><div class="event-name">${evt.nom}</div><div class="event-detail">${evt.lieu || ''} — ${evt.distance_km || ''} km</div></div>
...
async function sauvegarderEvenement() {
  ...
  if (!nom || !date) { alert('Le nom et la date sont obligatoires'); return }
  const { error } = await db.from('evenements').insert([{ nom, type, date, lieu, distance_km: parseFloat(distance) || null }])
  if (error) { console.error(error); return }
...
async function construireCalendrier() {
  ...
  const titreEl = document.getElementById('calendrier-titre')
  if (titreEl) titreEl.innerHTML = `${moisNoms[moisActuel]} <span>${anneeActuelle}</span>`
  ...
  for (let i = 0; i < decalage; i++) grid.appendChild(document.createElement('div'))
  const { data: evenements } = await db.from('evenements').select('date, type')
  ...
  for (let j = 1; j <= nbJours; j++) {
    ...
    if (j === aujourdhui.getDate() && moisActuel === aujourdhui.getMonth() && anneeActuelle === aujourdhui.getFullYear()) div.classList.add('today')
    if (evenements) {
      const evtDuJour = evenements.filter(e => e.date === dateStr)
      if (evtDuJour.length > 0) div.classList.add(evtDuJour[0].type === 'course' ? 'has-event' : 'has-event2')
    }
...
async function chargerClassement() {
  const { data, error } = await db.from('membres').select('nom, role, specialite, km_mois, points').order('km_mois', { ascending: false })
  if (error) { console.error('Erreur:', error); return }
  const liste = document.querySelector('#tab-km .member-list') || document.getElementById('classement-km-list')
  if (!liste) return
  liste.innerHTML = ''
  const medailles = ['🥇','🥈','🥉']
  const badges = ['','silver','bronze','other','other','other']
  data.forEach((membre, index) => {
    ...
    const largeur = Math.round((membre.km_mois / (data[0].km_mois || 1)) * 100)
    const div = document.createElement('div')
    div.className = 'member-row'
    div.innerHTML = `
      <div class="rank-badge ${badge}" style="width:28px;height:28px;font-size:13px">${medaille}</div>
      <div class="member-info"><div class="member-name">${membre.nom}</div><div class="member-role">${membre.specialite || membre.role || ''}</div></div>
      <div style="flex:1;margin:0 12px"><div style="height:8px;background:var(--surface2);border-radius:4px;overflow:hidden"><div style="height:100%;width:${largeur}%;background:var(--accent);border-radius:4px"></div></div></div>
...
async function chargerClassementPoints() {
  const { data, error } = await db.from('membres').select('nom, role, points').order('points', { ascending: false })
  if (error) { console.error(error); return }
  const liste = document.querySelector('#tab-pts .member-list') || document.getElementById('classement-pts-list')
  if (!liste) return
  liste.innerHTML = ''
  const medailles = ['🥇','🥈','🥉']
  const badges = ['','silver','bronze','other','other','other']
  data.forEach((membre, index) => {
    ...
    div.className = 'member-row'
    div.innerHTML = `
      <div class="rank-badge ${badges[index]||'other'}" style="width:28px;height:28px;font-size:13px">${medailles[index]||(index+1)}</div>
      <div class="member-info"><div class="member-name">${membre.nom}</div><div class="member-role">${membre.role||''}</div></div>
...
async function chargerParcours() {
  const { data, error } = await db.from('parcours').select('*').order('date_sortie', { ascending: false })
  if (error) { console.error('Erreur:', error); return }
  const liste = document.getElementById('parcours-list') || document.querySelector('#page-parcours .card')
  if (!liste) return
  liste.innerHTML = liste.id === 'parcours-list' ? '' : '<div class="card-title">Parcours récents</div>'
  const totalKm = data.reduce((sum, p) => sum + (parseFloat(p.distance_km) || 0), 0)
  const totalDenivele = data.reduce((sum, p) => sum + (parseInt(p.denivele_m) || 0), 0)
  if (document.getElementById('stat-km-parcours')) document.getElementById('stat-km-parcours').textContent = totalKm.toLocaleString()
  if (document.getElementById('stat-denivele')) document.getElementById('stat-denivele').textContent = totalDenivele.toLocaleString()
  if (document.getElementById('stat-sorties')) document.getElementById('stat-sorties').textContent = data.length
  const icones = ['🏔','🚴','⚡','🌄','🏁']
  ...
  data.forEach((parcours, index) => {
    ...
    const duree = parcours.duree_min ? `${Math.floor(parcours.duree_min/60)}h${String(parcours.duree_min%60).padStart(2,'0')}` : ''
    const div = document.createElement('div')
    div.className = 'route-item'
    div.innerHTML = `
    <div class="route-icon" style="background:${couleurs[index%couleurs.length]}">${icones[index%icones.length]}</div>
    <div class="route-data"><div class="route-name">${parcours.nom}</div><div class="route-meta">${dateStr} · ↑ ${parcours.denivele_m||0}m · ${duree}</div></div>
    <div><div class="route-km">${parcours.distance_km||0}</div><div class="route-unit">km</div></div>
...
async function sauvegarderParcours() {
  ...
  if (!nom || !date) { alert('Le nom et la date sont obligatoires'); return }
  const { error } = await db.from('parcours').insert([{
    nom, date_sortie: date,
    distance_km: parseFloat(document.getElementById('par-distance').value) || null,
    denivele_m: parseInt(document.getElementById('par-denivele').value) || null,
    duree_min: parseInt(document.getElementById('par-duree').value) || null
  }])
  if (error) { console.error(error); return }
  ['par-nom','par-date','par-distance','par-denivele','par-duree'].forEach(id => document.getElementById(id).value = '')
...
async function supprimerParcours(id) {
  ...
  const { error } = await db.from('parcours').delete().eq('id', id)
  if (error) { console.error(error); return }
  chargerParcours()
}
...
async function chargerDashboard() {
  ...
  const { data: membres } = await db.from('membres').select('km_mois').eq('actif', true)
  const { data: evenements } = await db.from('evenements').select('type, date').gte('date', debut).lte('date', fin)
  const { data: parcours } = await db.from('parcours').select('distance_km')
  ...
  if (document.getElementById('stat-km')) document.getElementById('stat-km').textContent = totalKm.toLocaleString()
  if (document.getElementById('stat-courses')) document.getElementById('stat-courses').textContent = coursesAvenir
  if (document.getElementById('stat-membres')) document.getElementById('stat-membres').textContent = totalMembres
  if (document.getElementById('stat-parcours')) document.getElementById('stat-parcours').textContent = totalParcours
}

async function chargerGraphique() {
  const { data, error } = await db.from('membres').select('nom, km_mois').order('km_mois', { ascending: false })
  if (error) { console.error(error); return }
  ...
  window.chartProgression = new Chart(document.getElementById('chart-progression'), {
    type: 'bar',
    data: { labels: noms, datasets: [{ label: 'Km ce mois', data: kms, backgroundColor: couleurs.slice(0, kms.length), borderRadius: 6 }] },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: '#6b7280' }, grid: { color: '#ffffff08' } }, y: { ticks: { color: '#6b7280' }, grid: { color: '#ffffff08' } } } }
  })
}
...
function connecterStrava() {
  window.location.href = `https://www.strava.com/oauth/authorize?client_id=${STRAVA_CLIENT_ID}&response_type=code&redirect_uri=${STRAVA_REDIRECT_URI}&approval_prompt=force&scope=activity:read_all`
}

async function recupererTokenStrava(code) {
  const { data, error } = await db.functions.invoke('strava-auth', { body: { code } })
  if (error) { console.error(error); return }
...
async function synchroniserActivitesStrava(token) {
  const response = await fetch('https://www.strava.com/api/v3/athlete/activities?per_page=10', { headers: { 'Authorization': `Bearer ${token}` } })
  const activites = await response.json()
  for (const activite of activites) {
    if (activite.type !== 'Ride') continue
    await db.from('parcours').insert([{ nom: activite.name, distance_km: Math.round(activite.distance/100)/10, denivele_m: Math.round(activite.total_elevation_gain), duree_min: Math.round(activite.moving_time/60), date_sortie: activite.start_date.split('T')[0] }])
  }
  chargerParcours()
...
async function importerMembresClub() {
  ...
  if (!token) { alert("Connectez d'abord Strava !"); return }
  const response = await fetch('https://www.strava.com/api/v3/clubs/940123/members?per_page=50', { headers: { 'Authorization': `Bearer ${token}` } })
  if (!response.ok) { alert('Erreur Strava'); return }
  const athletes = await response.json()
  let importes = 0
  for (const athlete of athletes) {
    const nom = `${athlete.firstname} ${athlete.lastname}`
    const { data: existants } = await db.from('membres').select('id').eq('nom', nom)
    if (existants && existants.length > 0) continue
    await db.from('membres').insert([{ nom, role: 'Équipier', specialite: 'Cycliste', km_mois: 0, points: 0, categorie: 'Elite' }])
    importes++
  }
  alert(`${importes} nouveaux membres importés !`)
  chargerMembres()
  chargerClassement()
  chargerClassementPoints()
}
...
async function synchroniserKmMembres() {
  const token = localStorage.getItem('strava_token')
  if (!token) { alert("Connectez d'abord Strava !"); return }
  let page = 1, toutesActivites = [], continuer = true
  while (continuer) {
    const response = await fetch(`https://www.strava.com/api/v3/clubs/940123/activities?per_page=100&page=${page}`, { headers: { 'Authorization': `Bearer ${token}` } })
    if (!response.ok) { alert('Erreur Strava'); return }
    const activites = await response.json()
    if (activites.length === 0 || activites.length < 100) continuer = false
  ...
  for (const [nom, km] of Object.entries(kmParMembre)) {
    const { data: membre } = await db.from('membres').select('id').eq('nom', nom)
    if (!membre || membre.length === 0) continue
    await db.from('membres').update({ km_mois: Math.round(km) }).eq('id', membre[0].id)
    mis_a_jour++
  }
  alert(`${mis_a_jour} membres mis à jour !\n${toutesActivites.length} activités analysées.`)
  chargerMembres()
  chargerClassement()
  chargerDashboard()
}
...
async function afficherStatsStrava() {
  const token = localStorage.getItem('strava_token')
  if (!token) return
  const response = await fetch('https://www.strava.com/api/v3/clubs/940123/activities?per_page=100', { headers: { 'Authorization': `Bearer ${token}` } })
  ...
  el.innerHTML = `
    <div style="background:#151820;border:1px solid #fc4c0240;border-radius:12px;padding:1rem;margin-bottom:1.5rem">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
        <div style="text-align:center"><div style="font-family:'Bebas Neue',sans-serif;font-size:24px;color:#fc4c02">${totalKm.toLocaleString()}</div><div style="font-size:11px;color:#6b7280">km total</div></div>
        <div style="text-align:center"><div style="font-family:'Bebas Neue',sans-serif;font-size:24px;color:#fc4c02">${totalDenivele.toLocaleString()}</div><div style="font-size:11px;color:#6b7280">m dénivelé</div></div>
        <div style="text-align:center"><div style="font-family:'Bebas Neue',sans-serif;font-size:24px;color:#fc4c02">${rides.length}</div><div style="font-size:11px;color:#6b7280">sorties</div></div>
        <div style="text-align:center"><div style="font-family:'Bebas Neue',sans-serif;font-size:24px;color:#fc4c02">${membres}</div><div style="font-size:11px;color:#6b7280">membres actifs</div></div>
      </div>
    </div>`
}
...
async function exporterPDF() {
  ...
  try { doc.addImage(logoCanvas.toDataURL('image/png'), 'PNG', 14, 5, 30, 30) } catch(e) {}
  doc.setTextColor(30, 120, 220); doc.setFontSize(22); doc.setFont('helvetica', 'bold')
  ...
  doc.setDrawColor(30, 120, 220); doc.line(14, 61, 196, 61)
  doc.setTextColor(0, 0, 0); doc.setFontSize(11); doc.setFont('helvetica', 'normal')
  doc.text(`Km ce mois : ${document.getElementById('stat-km').textContent} km`, 14, 73)
  doc.text(`Membres actifs : ${document.getElementById('stat-membres').textContent}`, 14, 83)
  doc.text(`Courses a venir : ${document.getElementById('stat-courses').textContent}`, 110, 73)
  doc.text(`Parcours enregistres : ${document.getElementById('stat-parcours').textContent}`, 110, 83)
  doc.setTextColor(30, 120, 220); doc.setFontSize(13); doc.setFont('helvetica', 'bold')
  doc.text('CLASSEMENT - KM CE MOIS', 14, 100)
  doc.setDrawColor(30, 120, 220); doc.line(14, 103, 196, 103)
  const { data: membres_data } = await db.from('membres').select('nom, role, km_mois, points, categorie, num_licence, date_naissance').order('km_mois', { ascending: false })
  doc.setFillColor(30, 120, 220); doc.rect(14, 106, 182, 8, 'F')
  doc.setTextColor(255, 255, 255); doc.setFont('helvetica', 'bold'); doc.setFontSize(10)
  doc.text('Rang', 16, 112); doc.text('Nom', 35, 112); doc.text('Date naissance', 95, 112); doc.text('Categorie', 145, 112); doc.text('Licence', 175, 112)
  doc.setFont('helvetica', 'normal')
  let y = 122
  membres_data.forEach((m, i) => {
    if (i % 2 === 0) { doc.setFillColor(235, 242, 255); doc.rect(14, y-5, 182, 8, 'F') }
    const dob = m.date_naissance ? new Date(m.date_naissance).toLocaleDateString('fr-FR') : ''
    doc.setTextColor(0, 0, 0)
    doc.text(`${i+1}`, 16, y); doc.text((m.nom||'').substring(0,22), 35, y); doc.text(dob, 95, y); doc.text(m.categorie||'', 145, y); doc.text(m.num_licence||'', 175, y)
    y += 10
    if (y > 260) { doc.addPage(); y = 20 }
  })
  const { data: evenements_data } = await db.from('evenements').select('*').order('date', { ascending: true })
  if (evenements_data && evenements_data.length > 0) {
    if (y > 220) { doc.addPage(); y = 20 }
    doc.setTextColor(30, 120, 220); doc.setFontSize(13); doc.setFont('helvetica', 'bold')
    doc.text('EVENEMENTS', 14, y+15); doc.setDrawColor(30, 120, 220); doc.line(14, y+18, 196, y+18)
    y += 28; doc.setFont('helvetica', 'normal'); doc.setFontSize(10)
    evenements_data.forEach(evt => {
      doc.setTextColor(0, 0, 0)
      doc.text(`${new Date(evt.date).toLocaleDateString('fr-FR')} - ${evt.nom} (${evt.type}) - ${evt.lieu||''} - ${evt.distance_km||''} km`, 14, y)
      y += 8
      if (y > 270) { doc.addPage(); y = 20 }
    })
  }
  doc.setFillColor(13, 15, 20); doc.rect(0, 285, 210, 12, 'F')
  doc.setTextColor(30, 120, 220); doc.setFontSize(8)
...
async function exporterEngagement() {
  ...
  const lieu = document.getElementById('eng-lieu').value
  ...
  const loadLogoRond = (src) => new Promise(resolve => {
    ...
    img.onload = () => {
      try { ctx.beginPath(); ctx.arc(size/2, size/2, size/2, 0, Math.PI*2); ctx.closePath(); ctx.clip(); ctx.drawImage(img, 0, 0, size, size) } catch(e) {}
  ...
  doc.setTextColor(30, 120, 220); doc.setFontSize(10); doc.setFont('helvetica', 'bold')
  ...
  arabicCanvas.width = 500; arabicCanvas.height = 32
  const arabicCtx = arabicCanvas.getContext('2d')
  arabicCtx.clearRect(0, 0, 500, 32); arabicCtx.fillStyle = '#1e78dc'
  arabicCtx.font = 'bold 20px Arial'; arabicCtx.textAlign = 'center'; arabicCtx.direction = 'rtl'
  ...
  doc.setTextColor(0, 0, 0); doc.setFontSize(24); doc.setFont('helvetica', 'bold')
  doc.text('ENGAGEMENT', 105, 46, { align: 'center' })
  doc.setFontSize(10); doc.setFont('helvetica', 'bold')
  doc.text(`Competition : ${competition}`, 14, 58)
  doc.setFont('helvetica', 'normal')
  doc.text(`Date : ${dateAff}`, 14, 67); doc.text(`Lieu : ${lieu}`, 110, 67)
  ...
  doc.setFont('helvetica', 'bold'); doc.text('LISTE DES COUREURS', 14, 113)
  ...
  const rowH = 8; let y = 118
  const colDos=14, colNom=30, colLic=118, colDob=150, colEm=178, endCol=196
  const drawRow = (yy) => {
    doc.rect(colDos, yy, endCol-colDos, rowH)
    doc.line(colNom, yy, colNom, yy+rowH); doc.line(colLic, yy, colLic, yy+rowH)
    doc.line(colDob, yy, colDob, yy+rowH); doc.line(colEm, yy, colEm, yy+rowH)
  }
  doc.setFillColor(200, 200, 200); doc.rect(colDos, y, endCol-colDos, rowH, 'F')
  doc.setDrawColor(0); doc.setLineWidth(0.3); drawRow(y)
  doc.setFontSize(8); doc.setFont('helvetica', 'bold')
  doc.text('N Dos', colDos+1, y+6); doc.text('Nom et Prenoms', colNom+1, y+6)
  doc.text('N Licence', colLic+1, y+6); doc.text('Date naissance', colDob+1, y+6); doc.text('Emargement', colEm+1, y+6)
  ...
  coureurs.forEach(c => {
    ...
    doc.text(c.num_licence||'', colLic+1, y+6); doc.text(dob, colDob+1, y+6)
    y += rowH; if (y > 265) { doc.addPage(); y = 20 }
  })

  y += 15; doc.setFont('helvetica', 'bold'); doc.setFontSize(12)
